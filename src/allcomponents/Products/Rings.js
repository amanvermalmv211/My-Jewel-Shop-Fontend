...
const Rings = (props) => {
    ...
    const [ring, setRing] = useState({ name: "Ring", imglink: "", weight: "", ktgold: "", makingcharge: "0", finalmakingcharge: "", gst: "3", quantity: "" });
    ...
    const handleLogin = async () => {
        if (localStorage.getItem("myjeweladmintoken")) {
            ...
            else if (!ring.finalmakingcharge) {
            ...
            try {
                ...
                setRing({ name: "Ring", imglink: "", weight: "", ktgold: "", makingcharge: "0", finalmakingcharge: "", gst: "3", quantity: "" });
            }
            catch (err) {
                toast.error('Internal Server Error Occured.', {
                    position: "top-center",
    ...
    return (
        <>
            <div className={`py-6 sm:py-8 lg:py-12 ${props.mode === "light" ? "text-gray-700" : "text-white"}`}>
                <div className="mx-auto max-w-screen-2xl px-4 md:px-8">
                    <div className='mb-10 md:mb-16'>
                        <h2 className={`${props.mode === "light" ? "text-gray-700" : "text-[#d4af37]"} mb-8 text-center text-2xl font-bold md:mb-6 lg:text-3xl`}>Rings</h2>
                        {localStorage.getItem("myjeweladmintoken") ? <div className='flex items-center justify-center p-4'>
                                ...
                                <div className={`rounded-md shadow-sm space-y-2 ${props.mode === "light" ? "text-gray-700" : "text-white"}`}>
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            required
                                            className="appearance-none rounded-md relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                                            placeholder="Enter the quantity of the Ring"
                                            value={ring.quantity}
                                            onChange={handleringChange}
                                        />
                                    </div>
                            ...
                            <p className={`mx-auto max-w-screen-md text-center md:text-lg`}>Welcome to our exquisite collection of Rings. Discover the perfect Ring to celebrate love, milestones, and personal style.</p>}
                    </div>
            ...
            {
                localStorage.getItem("myjewelacctoken") && <OrderCall mode={props.mode} jewel={"Ring"} />
            }
        </>
    )
}
