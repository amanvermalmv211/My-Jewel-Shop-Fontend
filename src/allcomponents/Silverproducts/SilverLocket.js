...
const SilverLocket = (props) => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = "Silver Lockets - MJS";

    try {
      getProduct("Silver Locket");
    }
    catch (err) {
      if (shouldDo.current) {
  ...
  const [silverlocket, setSilverlocket] = useState({ name: "Silver Locket", imglink: "", weight: "", ktgold: "", makingcharge: "0", finalmakingcharge: "", gst: "3", quantity: "" });

  const handlesilverlocketChange = (e) => {
    setSilverlocket({ ...silverlocket, [e.target.name]: e.target.value });
  };

  const handleLogin = async () => {
    if (localStorage.getItem("myjeweladmintoken")) {
      if (!silverlocket.name) {
      ...
      else if (!silverlocket.makingcharge) {
      ...
      else if (!silverlocket.finalmakingcharge) {
      ...
      try {
        ...
        setSilverlocket({ name: "Silver Locket", imglink: "", weight: "", ktgold: "", makingcharge: "0", finalmakingcharge: "", gst: "3", quantity: "" });
      }
      catch (err) {
        toast.error('Internal Server Error Occured.', {
          position: "top-center",
  ...
  return (
    <>
      <div className={`py-6 sm:py-8 lg:py-12 ${props.mode === "light" ? "text-gray-700" : "text-white"}`}>
        <div className="mx-auto max-w-screen-2xl px-4 md:px-8">
          <div className='mb-10 md:mb-16'>
            <h2 className={`${props.mode === "light" ? "text-gray-700" : "text-[#d4af37]"} mb-8 text-center text-2xl font-bold md:mb-6 lg:text-3xl`}>Silver Locket</h2>
            ...
            {localStorage.getItem("myjeweladmintoken") ? <div className='flex items-center justify-center p-4'>
              {isEditingAddress ? <form className="space-y-6 w-96" onSubmit={(e) => e.preventDefault()}>
                <div className={`rounded-md shadow-sm space-y-2 ${props.mode === "light" ? "text-gray-700" : "text-white"}`}>
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className='flex'>
                      <input
                        ...
                        value={silverlocket.imglink}
                        onChange={handlesilverlocketChange}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={silverlocket.makingcharge}
                      onChange={handlesilverlocketChange}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={silverlocket.finalmakingcharge}
                      onChange={handlesilverlocketChange}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="appearance-none rounded-md relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                      placeholder="Enter the quantity of the Silver Locket"
                      value={silverlocket.quantity}
                      onChange={handlesilverlocketChange}
                    />
                  </div>
                ...
                >
                  Add New Silver Locket
                </button>
              </div>}

            </div> :
              <p className={`mx-auto max-w-screen-md text-center md:text-lg`}>Unlock the door to a world where memories are held close to the heart. Our exquisite silver lockets are not just jewelry; they are a canvas for your unique story.</p>}
      ...
      {
        localStorage.getItem("myjewelacctoken") && <OrderCall mode={props.mode} jewel={"Silver Locket"} />
      }
    </>
  )
}

export default SilverLocket;