...
const GiftItems = (props) => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = "Gift Items - MJS";

    try {
      getProduct("Gift Item");
    }
    catch (err) {
      if (shouldDo.current) {
  ...
  const [giftitem, setGiftItem] = useState({ name: "Gift Item", imglink: "", weight: "", ktgold: "", makingcharge: "0", finalmakingcharge: "", gst: "3", quantity: "" });

  const handlegiftitemChange = (e) => {
    setGiftItem({ ...giftitem, [e.target.name]: e.target.value });
  };
  ...
  const handleLogin = async () => {
    if (localStorage.getItem("myjeweladmintoken")) {
      ...
      else if (!giftitem.finalmakingcharge) {
      ...
      try {
        ...
        setGiftItem({ name: "Gift Item", imglink: "", weight: "", ktgold: "", makingcharge: "0", finalmakingcharge: "", gst: "3", quantity: "" });
      }
      catch (err) {
        toast.error('Internal Server Error Occured.', {
          position: "top-center",
  ...
  return (
    <>
      <div className={`py-6 sm:py-8 lg:py-12 ${props.mode === "light" ? "text-gray-700" : "text-white"}`}>
        <div className="mx-auto max-w-screen-2xl px-4 md:px-8">
          <div className="mb-10 md:mb-16">
            <h2 className={`mb-4 text-center text-2xl font-bold ${props.mode === "light" ? "text-gray-700" : "text-[#d4af37]"} md:mb-6 lg:text-3xl`}>Gift Items</h2>
            ...
            {localStorage.getItem("myjeweladmintoken") ? <div className='flex items-center justify-center p-4'>
                ...
                <div className={`rounded-md shadow-sm space-y-2 ${props.mode === "light" ? "text-gray-700" : "text-white"}`}>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={giftitem.finalmakingcharge}
                      onChange={handlegiftitemChange}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="appearance-none rounded-md relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                      placeholder="Enter the quantity of the Gift Items"
              ...
              <p className={`mx-auto max-w-screen-md text-center md:text-lg`}>Discover a world of thoughtful gifts for every occasion. From birthdays to anniversaries, weddings to holidays, we have the perfect gift to make your loved ones smile.</p>}
      ...
      {
        localStorage.getItem("myjewelacctoken") && <OrderCall mode={props.mode} jewel={"Gift Item"} />
      }
